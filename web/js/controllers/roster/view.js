...
export function createRosterCard(rosterRecord, handlers = {}) {
  ...
  // Static structure: image area + body (title, subs, actions)
  cardElement.innerHTML = `
    ...
    <div class="card-body">
      ...
      ${subtitleLines
        .map((line) => `<div class="card-sub">${escapeHtml(line)}</div>`)
        .join("")}
    </div>
  `;

  // Card is clickable; also provide explicit Edit button.
...
function buildImageMarkup(src) {
  ...
  return `<img class="card-img-media" src="${escapeAttribute(
    cleaned
  )}&v=${Date.now()}" alt="" loading="lazy" decoding="async" data-roster-img="">`;
}
 ...
 * (We rely on `escapeHtml` for text nodes; this is only for attributes.)
 *
 * @param {string} value
 * @returns {string}
 */
